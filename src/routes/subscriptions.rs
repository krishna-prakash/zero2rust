...
async fn subscribe(
    ...
    let subscriber_id = match insert_subscriber(&new_subscriber, &connection).await {
...
async fn insert_subscriber(form: &NewSubscriber, connection: &PgPool) -> Result<Uuid, sqlx::Error> {
